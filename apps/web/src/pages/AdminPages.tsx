...
import type { Crop, Sensor, SensorReading } from '../types';
  ...
  return (
    <div className="page admin-page">
      ...
      <section className="admin-stats" aria-label="Estadísticas globales">
        <article><span className="admin-stat-icon"><UsersRound /></span><div><strong>{formatNumber(stats.users, 0)}</strong><span>Usuarios</span></div></article>
        <article><span className="admin-stat-icon"><Building2 /></span><div><strong>{formatNumber(stats.greenhouses, 0)}</strong><span>Invernaderos</span></div></article>
        <article><span className="admin-stat-icon"><Sprout /></span><div><strong>{formatNumber(stats.crops, 0)}</strong><span>Cultivos · {stats.activeCrops} activos</span></div></article>
        <article><span className="admin-stat-icon"><ThermometerSun /></span><div><strong>{formatNumber(stats.sensors, 0)}</strong><span>Sensores · {stats.activeSensors} activos</span></div></article>
        <article><span className="admin-stat-icon"><Activity /></span><div><strong>{formatNumber(stats.readings, 0)}</strong><span>Registros · {stats.readingsLast24h} en 24 h</span></div></article>
      </section>
      <div className="admin-overview-grid">
        <section className="panel admin-system-panel" aria-labelledby="system-title">
          ...
          <div className="system-ratio"><span>Recursos por usuario</span><strong>{stats.users ? formatNumber((stats.crops + stats.sensors) / stats.users) : '0'}</strong><small>Promedio de cultivos y sensores</small></div>
          <dl><div><dt>Sensores por invernadero</dt><dd>{stats.greenhouses ? formatNumber(stats.sensors / stats.greenhouses) : '0'}</dd></div><div><dt>Registros por sensor</dt><dd>{stats.sensors ? formatNumber(stats.readings / stats.sensors, 0) : '0'}</dd></div><div><dt>Tipos de sensor presentes</dt><dd>{stats.sensorsByType.length}</dd></div></dl>
        </section>
        <section className="panel recent-users" aria-labelledby="recent-users-title">
          ...
          {usersQuery.isPending ? <LoadingState label="Cargando usuarios…" /> : usersQuery.isError ? <ErrorState message={getErrorMessage(usersQuery.error)} onRetry={() => usersQuery.refetch()} /> : usersQuery.data.items.length ? <ul className="user-list">{usersQuery.data.items.map(({ user, counts }) => <li key={user.id}><span className="avatar">{user.name.slice(0, 1).toUpperCase()}</span><div><strong>{user.name}</strong><small>{user.email} · {counts.crops + counts.sensors} recursos</small></div><span className={`status status--${user.role === 'ADMIN' ? 'soil' : 'neutral'}`}>{user.role === 'ADMIN' ? 'Admin' : 'Usuario'}</span><Link className="icon-button" to={`/admin/users/${user.id}`} aria-label={`Ver ${user.name}`}><ArrowRight /></Link></li>)}</ul> : <EmptyState title="Sin usuarios" description="Las cuentas registradas aparecerán aquí." />}
        </section>
      </div>
    </div>
  );
...
export function AdminUserDetailPage() {
  ...
  const tabLabels: Record<ResourceType, string> = { crops: `Cultivos (${record.counts.crops})`, sensors: `Sensores (${record.counts.sensors})`, readings: `Registros (${record.counts.readings})` };
  return (
    <div className="page admin-page">
      <Link className="back-link" to="/admin/users"><ArrowLeft size={17} /> Volver a usuarios</Link>
      <header className="admin-user-hero"><div className="admin-user-hero__identity"><span className="avatar avatar--large">{user.name.slice(0, 1).toUpperCase()}</span><div><span className="eyebrow eyebrow--light">Cuenta registrada</span><h1>{user.name}</h1><p>{user.email}</p></div></div><dl><div><dt>Rol</dt><dd>{user.role === 'ADMIN' ? 'Administrador' : 'Usuario'}</dd></div><div><dt>Invernadero</dt><dd>{record.greenhouse?.name || 'Sin información'}</dd></div><div><dt>Registro</dt><dd>{formatDate(user.createdAt)}</dd></div></dl></header>
      <section className="panel resource-panel" aria-labelledby="resources-title">
        <div className="resource-panel__header"><div><span className="eyebrow">Solo lectura</span><h2 id="resources-title">Recursos asociados</h2></div><button className="button button--danger button--small" type="button" onClick={() => setDeleteOpen(true)}><Trash2 size={16} /> Eliminar cuenta</button></div>
